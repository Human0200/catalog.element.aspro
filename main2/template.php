...
<?php if (!empty($arResult['PROPERTIES']['OBLOJKA_DLYA_VIDEO']['VALUE'])): ?>
    <script>
...
<div class="item_main_info <?= (!$showCustomOffer ? "noffer" : ""); ?> <?= ($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props"); ?>" id="<?= $arItemIDs["strMainID"]; ?>">
    <div class="img_wrapper swipeignore">
        ...
        <div class="item_slider">
            <? if (!empty($arResult['PROPERTIES']['OBLOJKA_DLYA_VIDEO']['VALUE'])): ?>
                <? $posterPath = CFile::GetPath($arResult['PROPERTIES']['OBLOJKA_DLYA_VIDEO']['VALUE']); ?>
                <link rel="preload" as="image" href="<?= $posterPath ?>">
                <script>
                    window.__videoPoster = "<?= CUtil::JSEscape($posterPath) ?>";
                </script>
            <?php endif; ?>
                ...
                <? if ($showCustomOffer && !empty($arResult['OFFERS_PROP'])) { ?>
                    ...
                    if ($arResult["MORE_PHOTO"]) {
                        ...
                        if (!empty($arResult['PROPERTIES']['VIDEO_FILE']['VALUE'])) {
                            $videoWidth = $arResult['PROPERTIES']['VIDEO_FILE']['VALUE'][0]['width'];
                            $videoHeight = $arResult['PROPERTIES']['VIDEO_FILE']['VALUE'][0]['height'];
                            $videoPath = $arResult['PROPERTIES']['VIDEO_FILE']['VALUE'][0]['path'];
                            $posterPath = CFile::GetPath($arResult['PROPERTIES']['OBLOJKA_DLYA_VIDEO']['VALUE']);

                            $videoIframe = '<li class="video-slide' . ($showVideoFirst ? ' current' : '') . '">' .
                                '<div class="video-poster-container" ' .
                                'data-video-src="' . htmlspecialcharsbx($videoPath) . '" ' .
                                'data-poster="' . htmlspecialcharsbx($posterPath) . '" ' .
                                'style="margin: 0 auto; width: ' . $videoWidth . 'px; height: ' . $videoHeight . 'px; background: url(' . $posterPath . ') center/cover no-repeat; cursor: pointer; position: relative;">' .
                                '<div class="play-button-overlay" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 74px; height: 74px; background: rgba(0, 0, 0, 0.55); border-radius: 50%; display: flex; align-items: center; justify-content: center; pointer-events: none;">' .
                    ...
                    ?>
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const placeholders = document.querySelectorAll('.video-poster-container');
                                ...
                                placeholders.forEach(placeholder => {
                                    ...
                                    placeholder.addEventListener('click', function(e) {
                                        ...
                                        video.width = parseInt(width); // Убедимся, что числовое значение
                                        ...
                                        source.src = videoSrc;
                                        source.type = 'video/mp4'; // Убедитесь, что тип соответствует вашему видео

                                        video.appendChild(source);

                                        // Заменяем обложку на тег <video>
                                        this.replaceWith(video);

                                        // Автовоспроизведение
                                        video.play().catch(e => {
                                            console.warn("Autoplay blocked by browser policy. User gesture required.");
                                        });
                                    });
                                });
                            });
        ...
        <div class="info_item">
            ...
            <div class="middle_info main_item_wrapper">
                <? if ($arResult['PROPERTIES']['UF_DOP_PERCENT']['VALUE']) {
                    ...
                    $GLOBALS['customItemPercent'] = $GLOBALS['OPTIONS']['UF_ACTION_VALUE']['VALUE'];
                } ?>
            ...
            videoSlides.forEach(slide => {
                ...
                if (video) {
                    if (slide.classList.contains('current') || slide.classList.contains('flex-active-slide')) {
                        // --- АКТИВНЫЙ СЛАЙД ---
                        //console.log("Слайд активен, устанавливаем poster"); // Отладка
                        var poster = (window.__videoPoster || '').toString();
                        if (poster) {
                            // Проверяем, нужно ли устанавливать poster
                            const wasPosterSet = video.getAttribute('poster') && video.getAttribute('poster') === poster;
                            video.setAttribute('poster', poster); // Устанавливаем всегда, чтобы быть уверенным
                            
                                video.muted = true;
                            
                            // Если poster был установлен (или переустановлен), и видео на паузе,
                            // вызываем load(), чтобы помочь мобильному браузеру отобразить его.
                            // Это нужно, например, когда слайд снова становится активным.
                            if (video.paused) { // Проверяем, что видео на паузе
                                try {
                                    //console.log("Вызван video.load() для активного слайда для обновления poster"); // Отладка
                                    video.load(); // <-- Вот аккуратное добавление
                                } catch (e) {
                                    console.warn("Ошибка при вызове video.load():", e); // Отладка
                                }
                            }
                            // Если poster не менялся, load() не вызываем.
                            if (!wasPosterSet) {
                                //console.log("Poster был установлен впервые или изменился, load() вызван при паузе."); // Отладка
                            }
                        }
                    } else {
                        // --- НЕ АКТИВНЫЙ СЛАЙД ---
                        //console.log("Слайд не активен, ставим на паузу"); // Отладка
                        video.pause();
                        video.currentTime = 0;
                        // Убираем controls при смене слайда
    ...
    (function() {
        function setPosterOn(el, poster) {
            ...
            try {
                ...
                // УБИРАЕМ обработчик клика отсюда - пусть работает только MEDIA_COVER_REINIT_V2
            } catch (e) {}
        }
...
<script>
    ...
    (function() {
        ...
        function ensureBox(el) {
            var li = el.closest && el.closest('li.video-slide');
            var box = li || el.parentElement;
            if (!box) return null;
        ...
        function ensureBtn(box) {
            var btn = box.querySelector('.media-cover-playbtn');
            if (!btn) {
        ...
        function applyVideo(v) {
            ...
            if (v.paused || v.readyState === 0) show();
            else hide();

            if (!v._mc_bind) {
                // Обработчик клика на всю область видео (box)
                box.addEventListener('click', function(e) {
                    // Проверяем, что клик не был по самому видео с controls
                    if (e.target === v && v.hasAttribute('controls')) {
                        return; // Позволяем стандартному поведению video controls работать
                    }
        ...
        function applyIframe(f) {
            var box = ensureBox(f);
            if (!box) return;
            var btn = ensureBtn(box);
            var poster = (window.__videoPoster || '').toString();
            var img = box.querySelector('img.media-cover-img');
            ...
            if (!f.dataset.origSrc) {
                f.dataset.origSrc = f.getAttribute('src') || f.getAttribute('data-src') || '';
            }

            if (!f.getAttribute('src')) {
                btn.style.display = '';
                img.style.display = '';
                if (!box._mc_iframe_click) {
                    box.addEventListener('click', function(e) {
                        e.preventDefault();
                        var src = f.dataset.origSrc || '';
                        if (!src) return;
                        f.setAttribute('src', addAutoplay(src));
                        btn.style.display = 'none';
                        img.style.display = 'none';
                    });
                    box._mc_iframe_click = 1;
                }
            } else {
                btn.style.display = 'none';
                img.style.display = 'none';
        ...
        function applyAll() {
            toArr(document.querySelectorAll('li.video-slide video')).forEach(applyVideo);
            toArr(document.querySelectorAll('li.video-slide iframe, iframe#video-rutube, iframe#video-rutube-mobile')).forEach(applyIframe);
        }
        ...
        var gallery = document.querySelector('.item_slider');
        if (gallery && 'MutationObserver' in window) {
            new MutationObserver(applyAll).observe(gallery, {
                childList: true,
                subtree: true
            });
        }
...
<style>
    .media-cover-box {
        position: relative;
        cursor: pointer;
        /* Добавляем курсор pointer для всей области */
    }
    ...
    .media-cover-playbtn {
        ...
        z-index: 1000;
        pointer-events: none;
        /* Убираем pointer-events с кнопки, чтобы клики проходили через неё */
    }
