...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	$bChangeTitleItem = \Bitrix\Main\Config\Option::get('aspro.next', 'CHANGE_TITLE_ITEM', 'N') === 'Y';
...
if($arResult['PROPERTIES']['DESC_TABLE_BLOCK']){
	$descs = $arResult['PROPERTIES']['DESC_TABLE_BLOCK']['VALUE'];
	$iblockId = $arResult['PROPERTIES']['DESC_TABLE_BLOCK']['LINK_IBLOCK_ID'];

	$arSelect = Array();
	$arFilter = Array("IBLOCK_ID"=>IntVal($iblockId), "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID" => $descs);
	$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
	$arResult['SECTION_DESCR'] = '';
	while($ob = $res->GetNextElement())
	{
		$arFields = $ob->GetFields();
		$arResult['SECTION_DESCR'] .= $arFields['DETAIL_TEXT'];
	}
}
?>
<?
...
?>
<?
$optPriceGroup = Bitrix\Catalog\GroupTable::getRow(["filter" => ["NAME" => "OPT"]]);
$optPrice = Bitrix\Catalog\PriceTable::getRow(["filter" => ["PRODUCT_ID" => $arResult["ID"], "CATALOG_GROUP_ID" => $optPriceGroup["ID"]]]);
if ($optPrice) {
	$arResult["OPT_PRICE"] = $optPrice;
}

if (!empty($arResult["PROPERTIES"]["ID_TOVAROV_VKHODYASHCHIKH_V_SOSTAV_GRUPPY_OBEDINEN"]["VALUE"]) && !empty($arResult["PROPERTIES"]["NAZVANIE_SVOYSTVA_DLYA_OBEDINENIYA_PO_NEMU_V_GRUPP"]["VALUE"])) {
    $arResult["GROUP_BY"]["TITLE"] = $arResult["PROPERTIES"]["NAZVANIE_SVOYSTVA_DLYA_OBEDINENIYA_PO_NEMU_V_GRUPP"]["VALUE"];

    $productIdArray = [];
    $productIdArray[] = $arResult["ID"];
    $array = explode(",", $arResult["PROPERTIES"]["ID_TOVAROV_VKHODYASHCHIKH_V_SOSTAV_GRUPPY_OBEDINEN"]["VALUE"]);

    foreach ($array as $productIdValue) {
        $productIdValue = trim($productIdValue);
        if (!empty($productIdValue)) {
            $productIdArray[] = $productIdValue;
        }
    }
    ...
    $IBLOCK_ID = 17;
    $properties = CIBlockProperty::GetList(["sort"=>"asc"], ["IBLOCK_ID"=>$IBLOCK_ID, "NAME"=>$arResult["GROUP_BY"]["TITLE"]]);
    $propField = $properties->fetch();

    if ($propField) {
		
        $arResult["GROUP_BY"]["VALUES"] = [];
        $arFilter = ["ID"=>$productIdArray];
        $arSelect = ["ID", "IBLOCK_ID", "DETAIL_PAGE_URL", "PROPERTY_".$propField["CODE"]];
        $res = CIBlockElement::GetList(["SORT"=>"ASC","PROPERTY_".$propField["CODE"] => "ASC" ], $arFilter, false, false, $arSelect);
        $i = 0;
        while($ob = $res->GetNextElement())
        {
			
            $arFields = $ob->GetFields();
            $arResult["GROUP_BY"]["VALUES"][$i]["NAME"] = $arFields["PROPERTY_".$propField["CODE"]."_VALUE"];
            $arResult["GROUP_BY"]["VALUES"][$i]["LINK"] = $arFields["DETAIL_PAGE_URL"];
            $i++;
        }
    }
}